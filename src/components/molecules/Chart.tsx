...
import { buildParseTree } from "@/constants/node";
...
const Chart = () => {
  ...
  const getCellData = useCallback(
    (col: Symbol[] | null, rowIndex: number, colIndex: number) => {
      if (showMostRelevant) {
    ...
    [directory, getDisplayedNode, gridSize, showMostRelevant]
  );
    ...
    <>
      ...
      >
        {jsonArray?.map((json, index) => (
          <Tree key={index} data={json} orientation="vertical" collapsible />
        ))}
      </div>
